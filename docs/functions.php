...
function generate_id( $blog_id, $post, $order = '' ) {
	$attributes = array_filter( array( $blog_id, $post->post_type, $post->ID, $order ) );
 ...
 * @return array
 */
function serialize_record( $post, $post_attrs = array() ) {
	$blog_id       = get_current_blog_id();
	$split_content = split_text( $post->post_content );
	...
	$records = array_map(
		function ( $content, $index ) use ( $post, $post_attrs, $blog_id ) {
			return array_merge(
				...
				array(
					'content' => $content,
					'distinct_key' => generate_id( $blog_id, $post ),
				)
			);
		},
		$split_content,
