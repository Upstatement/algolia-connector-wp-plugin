<?php
/**
 * This is an example of integrating UpsAlgolia with
 * Editorial Starter Kit https://github.com/Upstatement/editorial-starter-wp-theme
 * Be sure to include and initialize AlgoliaManager in your functions.php file.
 *
 * @package YourPackageName
 */

namespace YourPackageName\Managers;

/** Class */
class AlgoliaManager {
	/**
	 * Add UpsAlgolia filters and Algolia environment variables to Timber context
	 *
	 * @return void
	 */
	public function run() {
		// UpsAlgolia credentials and configuration.
		add_filter( 'UpsAlgolia\get_algolia_application', array( $this, 'get_algolia_application' ) );
		add_filter( 'UpsAlgolia\get_algolia_settings', array( $this, 'get_algolia_settings' ) );
		add_filter( 'UpsAlgolia\get_index_name', array( $this, 'get_index_name' ) );
		add_filter( 'UpsAlgolia\is_indexable', array( $this, 'is_indexable' ), 10, 2 );

		// UpsAlgolia serializer functions.
		add_filter( 'UpsAlgolia\post_to_record', array( $this, 'post_to_record' ) );
		add_filter( 'UpsAlgolia\page_to_record', array( $this, 'page_to_record' ) );
	...
	/**
	 * Get Algolia index name. All posts use the same global index.
	 *
	 * @return string
	 */
	...
	private function split( $text ) {
		$char_limit = 1000;
		return str_split( wp_strip_all_tags( $text ), $char_limit );
	}
	 ...
	 * @return array
	 */
	private function serialize_record( $post, $post_attrs = array() ) {
		...
		$records = array_map(
			function ( $content, $index ) use ( $post, $post_attrs, $blog_id ) {
				return array_merge(
					$this->get_default_record_attributes(
						$post,
						$blog_id,
						$index + 1
					),
					...
					array(
						'content'      => $content,
						'distinct_key' => $this->generate_id( $blog_id, $post ),
					)
				);
			},
			$split_content,
