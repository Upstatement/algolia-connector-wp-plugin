<?php
/**
 * Description:     Exposes WP CLI commands for plugin
 *
 * @package         UpsAlgolia
 */
 ...
 * @return void
 */
function serialize_records( $index, $post_type ) {
	$paged = 1;
	$count = 0;
	...
	do {
		...
		try {
			...
			foreach ( $posts->posts as $post ) {
				...
				if ( $records ) {
					$serialized_records = array_merge(
						$serialized_records,
						$records
					);
					$count++;
...
class Algolia_Command {
	...
	public function reindex( $args, $assoc_args ) {
		...
		$blog_ids = $assoc_args['blog_id']
			? array( $assoc_args['blog_id'] )
		...
		foreach ( $blog_ids as $blog_id ) {
			...
			\WP_CLI::line(
				"\n"
				. 'Indexing posts from '
				. \WP_CLI::colorize( "%bBlog $blog_id%n" )
				. "\n"
			);
	...
	public function clear( $args, $assoc_args ) {
		...
		if ( count( $assoc_args ) ) {
			...
			\WP_CLI::line(
				'Clearing records with filters: '
				. \WP_CLI::colorize( "%b$filters%n" )
				. ' from '
				. \WP_CLI::colorize( "%p$index_name%n" )
			);

			$index->deleteBy( array( 'filters' => $filters ) )->wait();

		} else {
			\WP_CLI::line(
				'Clearing all records from '
				. \WP_CLI::colorize( "%p$index_name%n" )
			);
		...
		foreach ( $indices as $idx ) {
			...
			if ( isset( $assoc_args['settings'] ) ) {
				...
				if ( $settings ) {
					...
					\WP_CLI::success(
						'Pushed settings to ' . \WP_CLI::colorize( "%p$idx%n" )
					);
				}
			}
			...
			if ( isset( $assoc_args['synonyms'] ) ) {
				...
				if ( $synonyms ) {
					...
					\WP_CLI::success(
						'Pushed synonyms to ' . \WP_CLI::colorize( "%p$idx%n" )
					);
				}
			}
			...
			if ( isset( $assoc_args['rules'] ) ) {
				...
				if ( $rules ) {
					...
					\WP_CLI::success(
						'Pushed rules to ' . \WP_CLI::colorize( "%p$idx%n" )
					);
				}
			}
		}
